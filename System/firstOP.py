...
def get_first_cross(start_point, mas_for_points, count_m, n):
    ...
    flag = 0
    oop = start_point.copy()
    start_point = oop_to_mg(oop, params)
...
if __name__ == "__main__":
    ...
    axPoin.scatter(cross_points[0], cross_points[1], c=colors, cmap='plasma', s=2)  # viridis: синий → зелёный → жёлтый
    ...
    FP_oop = np.array([omega * np.cos(dzeta_-phi_), omega * np.sin(dzeta_-phi_),phi_])
    axPoin.scatter(FP_oop[0], FP_oop[1], c="red", s=20)
